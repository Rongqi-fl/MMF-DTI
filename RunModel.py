...
from utils.DataSetsFunction import CustomDataSet, collate_fn, FeatureCache
from utils.EarlyStoping import EarlyStopping
from LossFunction import  PolyLoss
...
def run_model(SEED, DATASET, MODEL, K_Fold, LOSS):
    ...
    drugstr_file = ('./Input/{}/drug_graph.pt'.format(DATASET))
    proteinstr_file = ('./Input/{}/protein_graph.pt'.format(DATASET))
    drugllm_file = ('./Input/{}/drug_llm.pt'.format(DATASET))
    proteinllm_file = ('./Input/{}/protein_llm.pt'.format(DATASET))

    cache = FeatureCache(drugstr_file, proteinstr_file, drugllm_file, proteinllm_file)
    ...
    for i_fold in range(K_Fold):
        ...
        train_dataset_loader = DataLoader(train_dataset, batch_size=hp.Batch_size, shuffle=True, num_workers=0,
                                          collate_fn=lambda batch_data: collate_fn(batch_data, cache, DATASET), drop_last=True)
        ...
        for epoch in range(1, hp.Epoch + 1):
            ...
            train_pbar = tqdm(
                ...
                total=len(train_dataset_loader), file=sys.stderr)

            """train"""
            train_losses_in_epoch = []
            model.train()
            for train_i, train_data in train_pbar:
                train_compounds, train_proteins,  train_drugs, train_sequences, train_labels = train_data
                train_compounds = train_compounds.to(DEVICE)
                train_proteins = train_proteins.to(DEVICE)
                train_labels = train_labels.to(DEVICE)
                train_drugs = train_drugs.to(DEVICE)
                train_sequences = train_sequences.to(DEVICE)

                optimizer.zero_grad()

                predicted_interaction = model(train_compounds, train_proteins, train_drugs, train_sequences)
            ...
            valid_pbar = tqdm(
                ...
                total=len(valid_dataset_loader), file=sys.stderr)
            ...
            with torch.no_grad():
                for valid_i, valid_data in valid_pbar:
                    ...
                    valid_compounds, valid_proteins, valid_drugs, valid_sequences, valid_labels = valid_data

                    valid_compounds = valid_compounds.to(DEVICE)
                    valid_proteins = valid_proteins.to(DEVICE)
                    valid_labels = valid_labels.to(DEVICE)
                    valid_drugs = valid_drugs.to(DEVICE)
                    valid_sequences = valid_sequences.to(DEVICE)

                    valid_scores = model(valid_compounds, valid_proteins,  valid_drugs, valid_sequences)
