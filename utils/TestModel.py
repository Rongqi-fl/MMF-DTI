...
def test_precess(MODEL, pbar, LOSS, DEVICE, FOLD_NUM):
    ...
    with torch.no_grad():
        for i, data in pbar:
            '''data preparation '''
            compounds, proteins,  drugs, sequences, labels = data
            ...
            if isinstance(MODEL, list):
                ...
                for i in range(len(MODEL)):
                    predicted_scores = predicted_scores + \
                        MODEL[i](compounds, proteins,  drugs, sequences)
                predicted_scores = predicted_scores / FOLD_NUM
            else:
                predicted_scores = MODEL(compounds, proteins, drugs, sequences)
...
def test_model(MODEL, dataset_loader, save_path, DATASET, LOSS, DEVICE, dataset_class="Train", save=True, FOLD_NUM=1):
    test_pbar = tqdm(
        ...
        total=len(dataset_loader), file=sys.stderr)
    ...
    print(results)
    filepath_csv = os.path.join(save_path, f"{DATASET}_{dataset_class}_prediction_results.csv")
    df_result = pd.DataFrame({
        'TrueLabel': T,
        'PredLabel': P,
        'Score': S  # softmax 后第1类的分数
    })
    df_result.to_csv(filepath_csv, index=False)
